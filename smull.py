...
def solve_and_print(grid: Sequence[Sequence[Optional[int]]],
    ...
    while slv.check() == sat:
        mdl = slv.model()
        Cbool, _, _ = evaluate_model(mdl, C, H, V, P)
    ...
    for i in range(nrow):
        for j in range(ncol):
            ...
            if i == 0 and (j == 0 or j == 1):
                ...
                slv.add(Implies(C[i][j], And(legal, Xor(absentH, absentV)))) ###### Legality is redundant, but I want to be sure.
            # If horizontal link of unshaded square points left, j > 0; and if it points right, j < ncol - 1
            slv.add(And(Implies(And(C[i][j], H[i][j][0]), j > 0), Implies(And(C[i][j], H[i][j][1]), j < ncol - 1)))
            # If vertical link of unshaded square points up, i > 0 and if it points down, i < nrow - 1
            slv.add(And(Implies(And(C[i][j], V[i][j][0]), i < nrow - 1), Implies(And(C[i][j], V[i][j][1]), i > 0)))
